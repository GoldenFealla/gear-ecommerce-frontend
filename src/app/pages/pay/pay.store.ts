...
@Injectable()
export class PayStore extends ComponentStore<PayState> {
    ...
    // *********** Effects ************* //
    payToCart = this.effect<{ id: string; success?: () => void }>(
        (trigger$) => {
            return trigger$.pipe(
                tap(() => this.setProcessCart()),
                exhaustMap(({ id, success }) =>
                    this.orderService.payCart(id).pipe(
                        tapResponse({
                            next: () => {
